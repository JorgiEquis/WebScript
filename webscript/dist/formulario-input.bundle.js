...
function create_campoTexto(state, effect, props = {}) {
  const __el0 = document.createElement("input");
  __el0.setAttribute("placeholder", "Escribe algo");
  __el0.addEventListener("input", (event) => {
    state.texto = event.target.value
  });
  return __el0;
}

function create_botonEnviar(state, effect, props = {}) {
  const __el0 = document.createElement("button");
  __el0.setAttribute("class", "boton");
  __el0.addEventListener("click", async (event) => {
    var r = await postController({ texto: state.texto })
        state.total = r.total
        state.texto = ""
  });
  const __el1 = document.createTextNode("Enviar");
...
function create_pagina(state, effect, props = {}) {
  ...
  const __el1 = create_campoTexto(state, effect, {  });
  __el0.appendChild(__el1);
  const __el2 = create_botonEnviar(state, effect, {  });
  __el0.appendChild(__el2);
  const __el3 = document.createElement("p");
  const __el4 = document.createTextNode("Total enviados: ");
  __el3.appendChild(__el4);
  const __el5 = document.createTextNode('');
  effect(() => { __el5.textContent = state.total; });

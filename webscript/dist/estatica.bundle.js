...
function create_paginaEstatica(state, effect, props = {}) {
  const __el0 = document.createElement("div");
  __el0.setAttribute("class", "boton");
  __el0.addEventListener("click", (event) => {
    state.contador++
  });
  const __el1 = document.createElement("h1");
  const __el2 = document.createTextNode("Ruta estática");
  __el1.appendChild(__el2);
  __el0.appendChild(__el1);
  const __el3 = document.createElement("p");
  const __el4 = document.createTextNode("Contador: ");
  __el3.appendChild(__el4);
  const __el5 = document.createTextNode('');
  effect(() => { __el5.textContent = state.contador; });
  __el3.appendChild(__el5);
  __el0.appendChild(__el3);
  const __el6 = document.createComment('if');
  const __el7 = document.createDocumentFragment();
  __el7.appendChild(__el6);
  let __el8_current = [];
  effect(() => {
    __el8_current.forEach(n => n.remove());
    __el8_current = [];
    if (state.contador == 0) {
      const __el9 = document.createElement("p");
      const __el10 = document.createTextNode("Todavia no le has dado al boton");
      __el9.appendChild(__el10);
      __el8_current = [__el9];
    }
    else if (state.contador < 5) {
      const __el11 = document.createElement("p");
      const __el12 = document.createTextNode("Vas bien");
      __el11.appendChild(__el12);
      __el8_current = [__el11];
    }
    else {
      const __el13 = document.createElement("p");
      const __el14 = document.createTextNode("Ya llevas unos cuantos");
      __el13.appendChild(__el14);
      __el8_current = [__el13];
    }
    const __ref = __el6.nextSibling;
    __el8_current.forEach(n => __el6.parentNode.insertBefore(n, __ref));
  });
  __el0.appendChild(__el7);
  return __el0;
}
...
let __init_contador = 0;
const { store: state, effect } = createStore({
  contador: __init_contador
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  app.appendChild(create_paginaEstatica(state, effect, {}));
});

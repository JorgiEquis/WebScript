...
function create_pagina(state, effect, props = {}) {
  ...
  __el0.addEventListener("click", async (event) => {
    var r = await postController({ precio: 100 })
        state.resultado = r.conIva
        state.total = r.totalConIva
  });
  const __el1 = document.createElement("h1");
  const __el2 = document.createTextNode("Con server function");
  __el1.appendChild(__el2);
  __el0.appendChild(__el1);
  const __el3 = document.createElement("p");
  const __el4 = document.createTextNode("Resultado: ");
  __el3.appendChild(__el4);
  const __el5 = document.createTextNode('');
  effect(() => { __el5.textContent = state.resultado; });
  __el3.appendChild(__el5);
  __el0.appendChild(__el3);
  const __el6 = document.createElement("p");
  const __el7 = document.createTextNode("Total acumulado: ");
  __el6.appendChild(__el7);
  const __el8 = document.createTextNode('');
  effect(() => { __el8.textContent = state.total; });
  __el6.appendChild(__el8);
  __el0.appendChild(__el6);
  return __el0;
}
...
// Llama a "post function postController" en el servidor -- POST a la URL de esta
// misma ruta.
...
async function postController(args) {
  if (location.protocol === 'file:') {
    throw new Error('"postController" necesita un servidor -- abre esta página vía http://, no como archivo local (file://). Usa: node src/cli.js run <carpeta> --serve');
  }
  var __url = "/con-server-function";
  return fetch(__url, {
    method: 'POST',
...
async function __wsInit() {
  if (location.protocol === 'file:') {
    document.getElementById('app').innerHTML =
      ...
      '<code>http://localhost:3000' + "/con-server-function" + '</code> en el navegador.' +
      '</div>';
    return;
  }

  server = await fetch("/con-server-function.server-data.json").then(r => r.json());

  let __init_resultado = 0;
  let __init_total = server.totalConIva;
  const store = createStore({
  resultado: __init_resultado,
  total: __init_total
  });
  state = store.store;
  effect = store.effect;
  ...
  app.appendChild(create_pagina(state, effect, {}));
}

...
function create_paginaFormulario(state, effect, props = {}) {
  ...
  __el0.addEventListener("click", async (event) => {
    var resultado = await postController({ cantidad: 5 })
        state.total = resultado.totalPedidos
        state.mensaje = resultado.mensaje
  });
  const __el1 = document.createElement("h1");
  const __el2 = document.createTextNode("Formulario");
  __el1.appendChild(__el2);
  __el0.appendChild(__el1);
  const __el3 = document.createElement("p");
  const __el4 = document.createTextNode("Total acumulado: ");
  ...
  const __el7 = document.createTextNode('');
  effect(() => { __el7.textContent = state.mensaje; });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  app.appendChild(create_paginaFormulario(state, effect, {}));
});
